...
const DEBUG = false;
export const PrivateToDer = (privateKeyHex) => {
    if (DEBUG) {
...
export const GetHash = (encodedTx, debug) => {
    return Buffer.from(encodedTx, 'hex');
};
export const Sign = (encodedTx, privateKeyHex) => {
    if (DEBUG) {
...
export const Verify = (encodedTx, signatureHex, publicKeyHex) => {
    if (DEBUG) {
...
export const GetPublicFromPrivate = (privateKey) => {
    ...
    if (encodedHex.startsWith(PUBLIC_KEY_PREFIX)) {
        ...
        throw new Error(`unknown prefix, expecting '${PUBLIC_KEY_PREFIX}' cannot decode public key '${encodedHex}'`);
    }
};
