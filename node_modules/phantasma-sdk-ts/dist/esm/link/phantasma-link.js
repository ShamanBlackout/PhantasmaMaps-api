...
export class PhantasmaLink {
    ...
    signTx(script, payload, callback, onErrorCallback, pow = ProofOfWork.None, signature = 'Ed25519') {
        //Overload Protection
    ...
    fetchWallet(callback, onErrorCallback) {
    ...
    getWalletVersion(callback, onErrorCallback) {
        this.onError = onErrorCallback; //Sets Error Callback Function
    ...
    createSocket(isResume = false) {
        ...
        const socket = useInjectedSocket && injectedSocket ? new injectedSocket() : new WebSocket(path);
        this.socket = socket;
        this.requestCallback = null;
        this.lastSocketErrorMessage = null;
        this.token = null;
                    ...
                    if (result.success) {
                        ...
                        this.sendLinkRequest(getAccountRequest, (result) => {
                            if (result.success) {
                                ...
                                this.onError?.('Could not obtain account info... Make sure you have an account currently open in ' +
                                    this.wallet +
                                    '...');
        ...
        socket.onmessage = (event) => {
            ...
            switch (obj.message) {
                case 'Wallet is Closed':
                    ...
                    break;
                case 'not logged in':
                    this.onError?.('Could not obtain account info... Make sure you have an account currently logged in');
                    this.disconnect(true);
                    break;
    ...
    sendLinkRequest(request, callback) {
        ...
        const isSocketOpen = hasSend && (hasReadyState ? socket.readyState === openState : this.socketOpen);
