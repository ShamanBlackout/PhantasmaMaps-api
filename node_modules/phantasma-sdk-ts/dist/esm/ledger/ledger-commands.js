...
import { getLedgerChainName, getLedgerDebug, getLedgerNexusName, getLedgerPayload, getLedgerRpc, getLedgerTransport, getLedgerVerifyResponse, } from './interfaces/ledger-config.js';
 ...
 * @returns
 */
export const getLedgerDeviceInfo = async (config) => {
    if (config == undefined) {
 ...
 * @returns
 */
export const getLedgerAccountSigner = async (config, accountIx) => {
    /* istanbul ignore if */
    if (config === undefined) {
        throw Error('config is a required parameter.');
    }
 ...
 * @returns
 */
export async function getLedgerSignerData(config, options) {
    if (config == undefined) {
        throw Error('config is a required parameter.');
    }
 ...
 * @returns
 */
export const getBalanceFromLedger = async (config, options) => {
    /* istanbul ignore if */
    if (config == undefined) {
        throw Error('config is a required parameter.');
    }
    ...
    if (rpcResponse.balances !== undefined) {
        rpcResponse.balances.forEach((balanceElt) => {
            response.balances?.set(balanceElt.symbol, toWholeNumber(balanceElt.amount, balanceElt.decimals));
        });
    }
    response.address = address;
    response.publicKey = publicKey;
 ...
 * @returns
 */
export const getAddressFromLedger = async (config, options) => {
    /* istanbul ignore if */
    if (config == undefined) {
        throw Error('config is a required parameter.');
    }
 ...
 * @returns
 */
export async function sendTransactionLedger(config, script) {
    if (config == undefined) {
        throw Error('config is a required parameter.');
    }
    const debug = getLedgerDebug(config);
    ...
    const myTransaction = new Transaction(nexusName, // Nexus Name
    chainName, // Chain
 ...
 * @returns
 */
export const getBalanceFromMnemonic = async (config, mnemonic, index) => {
    /* istanbul ignore if */
    if (config == undefined) {
        throw Error('config is a required parameter.');
    }
