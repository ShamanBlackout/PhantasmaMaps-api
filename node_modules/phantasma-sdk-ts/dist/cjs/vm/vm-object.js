...
const timestamp_js_1 = require("../types/timestamp.js");
...
const phantasma_big_int_serialization_js_1 = require("../types/phantasma-big-int-serialization.js");
...
class VMObject {
    ...
    getChildren() {
        return this.Type == vm_type_js_1.VMType.Struct ? this.Data : null;
    }
    get size() {
        let total = 0;
        if (this.Type == vm_type_js_1.VMType.Object) {
    ...
    constructor() {
        this._localSize = 0;
        this.Type = vm_type_js_1.VMType.None;
        this.Data = null;
    }
    static bytesFromAny(value) {
        ...
        if (typeof value === 'string') {
            return index_js_1.Base16.decodeUint8Array(value);
        }
        throw new Error(`Cannot convert ${typeof value} to bytes`);
    }
    static bigIntFromAny(value) {
    ...
    static serializeToBytes(value) {
        const writer = new index_js_1.PBinaryWriter();
    ...
    getArrayValue(index) {
        ...
        for (const [key, value] of children) {
            if (key.Type === vm_type_js_1.VMType.Number && key.asNumber() === BigInt(index)) {
                return value;
            }
        }
        return null;
    ...
    asTimestamp() {
        if (this.Type != vm_type_js_1.VMType.Timestamp) {
    ...
    asByteArray() {
        switch (this.Type) {
            case vm_type_js_1.VMType.Bytes:
            ...
            case vm_type_js_1.VMType.Number:
                return (0, phantasma_big_int_serialization_js_1.bigIntToTwosComplementLE_phantasma)(this.asNumber());
            case vm_type_js_1.VMType.Enum:
                const num = Number(this.asNumber());
                const bytes = new Uint8Array(4);
                new DataView(bytes.buffer).setUint32(0, num, true);
                return bytes;
            case vm_type_js_1.VMType.Timestamp:
            ...
            case vm_type_js_1.VMType.Object:
                if (this.Data instanceof index_js_1.Address) {
    ...
    asString() {
        switch (this.Type) {
            ...
            case vm_type_js_1.VMType.Bytes:
                return (0, index_js_3.uint8ArrayToStringDefault)(this.Data);
            case vm_type_js_1.VMType.Enum:
                return this.Data.toString();
            case vm_type_js_1.VMType.Object:
                if (this.Data instanceof index_js_1.Address) {
            ...
            case vm_type_js_1.VMType.Struct:
                ...
                if (arrayType === vm_type_js_1.VMType.Number) {
    ...
    asNumber() {
        switch (this.Type) {
            ...
            case vm_type_js_1.VMType.Bytes: {
                ...
                return (0, carbon_serialization_js_1.twosComplementLEToBigInt)(VMObject.bytesFromAny(this.Data));
            }
            case vm_type_js_1.VMType.Enum: {
            ...
            case vm_type_js_1.VMType.Object: {
                if (this.Data instanceof timestamp_js_1.Timestamp) {
                    return BigInt(this.Data.value);
                }
                if (this.Data instanceof index_js_1.Address) {
                ...
                if (bytes.length === 32) {
                    ...
                    return (0, carbon_serialization_js_1.twosComplementLEToBigInt)(bytes);
                }
                throw new Error(`Invalid cast: expected number, got ${this.Type}`);
            }
            default:
    ...
    asEnum() {
        ...
        if (this.Type !== vm_type_js_1.VMType.Enum) {
            this.Data = Number(this.asNumber());
        }
        return this.Data;
    }
    getArrayType() {
        if (this.Type !== vm_type_js_1.VMType.Struct) {
            return vm_type_js_1.VMType.None;
        }
        const children = this.getChildren();
        let result = vm_type_js_1.VMType.None;
        for (let i = 0; i < children.size; i++) {
            const val = this.getArrayValue(i);
            if (!val) {
                return vm_type_js_1.VMType.None;
            }
            if (result === vm_type_js_1.VMType.None) {
                result = val.Type;
            }
    ...
    static isEnum(instance) {
        if (instance == null)
            return false;
        const enumLike = instance;
    ...
    static isStructOrClass(type) {
        return ((!VMObject.isPrimitive(type) && VMObject.isValueType(type) && !VMObject.isEnum(type)) ||
            VMObject.isClass(type) ||
            VMObject.isInterface(type));
    }
    static isSerializable(type) {
        return (type instanceof index_js_2.ISerializable ||
            (0, index_js_2.isSerializableLike)(type) ||
    ...
    static isInterface(type) {
        return type === Map;
    }
    static ConvertObjectInternal(fieldValue, fieldType) {
        if ((VMObject.isStructOrClass(fieldType) && fieldValue instanceof Uint8Array) ||
            fieldValue instanceof Array) {
            const bytes = fieldValue instanceof Uint8Array ? fieldValue : Uint8Array.from(fieldValue);
            fieldValue = index_js_1.Serialization.deserialize(bytes, typeof fieldType);
        }
    ...
    toArray(arrayElementType) {
        ...
        for (const child of children) {
            if (child[0].Type !== vm_type_js_1.VMType.Number) {
    ...
    toObject() {
        if (this.Type === vm_type_js_1.VMType.None) {
    ...
    toStruct(structType) {
        ...
        const localType = new structType();
        ...
        let fields2 = index_js_1.Describer.describe(structType, false);
    ...
    static getVmType(type) {
        const typeName = typeof type === 'string' ? type.toLowerCase() : '';
        if (VMObject.isEnum(type)) {
            return vm_type_js_1.VMType.Enum;
        }
        if (type === Boolean || typeName === 'boolean') {
            return vm_type_js_1.VMType.Bool;
        }
        if (type === String || typeName === 'string') {
            return vm_type_js_1.VMType.String;
        }
        if (type === Uint8Array || typeName === 'uint8array') {
            return vm_type_js_1.VMType.Bytes;
        }
        if (type === 'BigInt' || type === Number || type === BigInt || typeName === 'number') {
            return vm_type_js_1.VMType.Number;
        }
        if (type === timestamp_js_1.Timestamp || type === Number) {
            return vm_type_js_1.VMType.Timestamp;
        }
        if (VMObject.isEnum(type)) {
            return vm_type_js_1.VMType.Enum;
        }
        if (Array.isArray(type)) {
            return vm_type_js_1.VMType.Struct;
        }
        if (VMObject.isClass(type) || VMObject.isValueType(type)) {
            return vm_type_js_1.VMType.Object;
        }
        return vm_type_js_1.VMType.Struct;
    }
    static isVmType(type) {
        const result = VMObject.getVmType(type);
        return result !== vm_type_js_1.VMType.None;
    }
    setValue(val, type) {
        if (type === undefined) {
            this.Data = val;
            if (val instanceof VMObject) {
                ...
                this.Type = vm_type_js_1.VMType.Struct;
            }
            else if (val instanceof Uint8Array || Array.isArray(val)) {
                this.Type = vm_type_js_1.VMType.Bytes;
            }
            else if (val instanceof timestamp_js_1.Timestamp) {
                this.Type = vm_type_js_1.VMType.Timestamp;
            }
            else if (val instanceof index_js_1.Address) {
                this.Type = vm_type_js_1.VMType.Object;
            }
            else if ((0, index_js_2.isSerializableLike)(val)) {
                this.Type = vm_type_js_1.VMType.Object;
            }
            else if (typeof val === 'bigint' || typeof val === 'number') {
                this.Type = vm_type_js_1.VMType.Number;
                this.Data = BigInt(val);
            }
            else if (typeof val === 'string' || val instanceof String) {
                this.Type = vm_type_js_1.VMType.String;
                this.Data = val.toString();
            }
            else if (typeof val === 'boolean' || val instanceof Boolean) {
                this.Type = vm_type_js_1.VMType.Bool;
        ...
        switch (type) {
            ...
            case vm_type_js_1.VMType.Enum:
                ...
                break;
            case vm_type_js_1.VMType.Timestamp:
                if (val instanceof timestamp_js_1.Timestamp) {
                    this.Data = val;
                }
                    ...
                    this.Data = new timestamp_js_1.Timestamp(value);
                }
                else {
                    this.Data = new timestamp_js_1.Timestamp(Number(val));
                }
                break;
            case vm_type_js_1.VMType.Bool:
                this.Data =
            ...
            default:
                if (val instanceof Uint8Array) {
                    ...
                    switch (len) {
                        case index_js_1.Address.lengthInBytes:
                            this.Data = index_js_1.Address.fromBytes(val);
                            break;
                        /*case Hash.Length:
    ...
    static validateStructKey(key) {
        if (key.Type == vm_type_js_1.VMType.None || key.Type == vm_type_js_1.VMType.Struct || key.Type == vm_type_js_1.VMType.Object) {
            throw new Error(`Cannot use value of type ${key.Type} as key for struct field`);
        }
    }
    castViaReflection(srcObj, level, dontConvertSerializables = true) {
        ...
        if (Array.isArray(srcObj)) {
            ...
            let isKnownType = srcVmType === vm_type_js_1.VMType.Number || srcVmType === vm_type_js_1.VMType.Timestamp;
            ...
            if (VMObject.isStructOrClass(localType) && !isKnownType) {
                ...
                if (fields.length > 0) {
                    ...
                    result = new VMObject();
                    result.setValue(children);
                    result.Type = vm_type_js_1.VMType.Struct;
                    return result;
                }
            }
    ...
    setKey(key, obj) {
        ...
        else if (this.Type == vm_type_js_1.VMType.None) {
            this.Type = vm_type_js_1.VMType.Struct;
            children = new Map();
            this.Data = children;
            this._localSize = 0;
    ...
    copy(other) {
        if (!other || other.Type == vm_type_js_1.VMType.None) {
            this.Type = vm_type_js_1.VMType.None;
            this.Data = null;
            return;
        }
        this.Type = other.Type;
        if (other.Type == vm_type_js_1.VMType.Struct) {
            const children = new Map();
            const otherChildren = other.getChildren();
            otherChildren?.forEach((val, key) => {
                const temp = new VMObject();
                temp.copy(val);
    ...
    static castTo(srcObj, type) {
        ...
        switch (type) {
            ...
            case vm_type_js_1.VMType.String: {
                const result = new VMObject();
                result.setValue(srcObj.asString(), vm_type_js_1.VMType.String);
                return result;
            }
            case vm_type_js_1.VMType.Timestamp: {
                const result = new VMObject();
                result.setValue(srcObj.asTimestamp().value, vm_type_js_1.VMType.Timestamp);
                return result;
            }
            case vm_type_js_1.VMType.Bool: {
                const result = new VMObject();
                result.setValue(srcObj.asBool(), vm_type_js_1.VMType.Bool);
                return result;
            }
            case vm_type_js_1.VMType.Bytes: {
                const result = new VMObject();
                result.setValue(srcObj.asByteArray(), vm_type_js_1.VMType.Bytes);
                return result;
            }
            case vm_type_js_1.VMType.Number: {
                const result = new VMObject();
                result.setValue(srcObj.asNumber(), vm_type_js_1.VMType.Number);
                return result;
            }
            case vm_type_js_1.VMType.Struct: {
                switch (srcObj.Type) {
    ...
    static fromObject(obj) {
        ...
        if (type === vm_type_js_1.VMType.None) {
            throw new Error('not a valid object');
        }
        const result = new VMObject();
        switch (type) {
            case vm_type_js_1.VMType.Bool:
                result.setValue(obj, vm_type_js_1.VMType.Bool);
                break;
            case vm_type_js_1.VMType.Bytes:
                result.setValue(obj, vm_type_js_1.VMType.Bytes);
                break;
            case vm_type_js_1.VMType.String:
                result.setValue(obj, vm_type_js_1.VMType.String);
                break;
            case vm_type_js_1.VMType.Enum:
                result.setValue(obj, vm_type_js_1.VMType.Enum);
                break;
            case vm_type_js_1.VMType.Object:
                result.setValue(obj, vm_type_js_1.VMType.Object);
                break;
            case vm_type_js_1.VMType.Number:
                /*if (objType === Number) {
                  obj = BigInt(obj);
                }*/
                result.setValue(obj, vm_type_js_1.VMType.Number);
                break;
            case vm_type_js_1.VMType.Timestamp:
                /*if (objType === "Number") {
                  obj = new Timestamp(obj);
                }*/
                result.setValue(obj, vm_type_js_1.VMType.Timestamp);
                break;
            case vm_type_js_1.VMType.Struct:
                result.Type = vm_type_js_1.VMType.Struct;
    ...
    static fromEnum(obj) {
        ...
        vm.setValue(obj, vm_type_js_1.VMType.Enum);
        return vm;
    }
    static fromStruct(obj) {
        const vm = new VMObject();
    ...
    static fromBytes(bytes) {
        const result = new VMObject();
        const reader = new index_js_1.PBinaryReader(bytes);
    ...
    serializeData(writer) {
        ...
        switch (this.Type) {
            ...
            case vm_type_js_1.VMType.Object: {
                ...
                const inner = new index_js_1.PBinaryWriter();
                if (this.Data instanceof index_js_1.Address) {
                ...
                else if ((0, index_js_2.isSerializableLike)(this.Data)) {
                    (0, index_js_2.serializeSerializable)(this.Data, inner);
                }
                else {
    ...
    unserializeData(reader) {
        ...
        switch (this.Type) {
            ...
            case vm_type_js_1.VMType.Object:
                ...
                if (bytes.length == index_js_1.Address.lengthInBytes + 1 && bytes[0] == index_js_1.Address.lengthInBytes) {
                    this.Data = index_js_1.Address.fromBytes(bytes.slice(1));
                    this.Type = vm_type_js_1.VMType.Object;
                }
                else {
                    this.Type = vm_type_js_1.VMType.Bytes;
                ...
                break;
            case vm_type_js_1.VMType.Enum:
                this.Type = vm_type_js_1.VMType.Enum;
