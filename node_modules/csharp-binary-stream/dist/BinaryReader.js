...
class BinaryReader {
    ...
    set position(value) {
        if (typeof value !== 'number'
            ...
            throw new InvalidArgumentError_1.InvalidArgumentError("Cannot set position to a non-numeric value", 'position', value);
        }
     ...
     */
    constructor(stream, endianness = Endianness_1.Endianness.Little) {
        if (stream instanceof ArrayBuffer || stream instanceof Uint8Array) {
            this._view = new Uint8Array(stream);
        }
        else {
            throw new InvalidArgumentError_1.InvalidArgumentError("`stream` must be either an instance of ArrayBuffer or Uint8Array", 'stream', stream);
        }
    ...
    readShort() {
        ...
        this._position += 2;
        const short = this.endianness === Endianness_1.Endianness.Little
            ? byte2 * 256 + byte1
            : byte1 * 256 + byte2;
        return short < 32768
            ? short
    ...
    readUnsignedShort() {
        ...
        const byte2 = this._view[this._position + 1];
        this._position += 2;
        return this.endianness === Endianness_1.Endianness.Little
            ? byte2 * 256 + byte1
            : byte1 * 256 + byte2;
    }
    /**
    ...
    readInt() {
        ...
        this._position += 4;
        const int = this.endianness === Endianness_1.Endianness.Little
            ? byte4 * 256 * 256 * 256
                + byte3 * 256 * 256
                + byte2 * 256
                + byte1
    ...
    readLongString() {
        ...
        const long = this.endianness === Endianness_1.Endianness.Little
            ? BigInt(byte1)
                + (BigInt(byte2) * m256)
    ...
    readFloat() {
        ...
        if (this.endianness === Endianness_1.Endianness.Little) {
     ...
     * @see [C# `BinaryReader.ReadDouble` documentation](https://docs.microsoft.com/en-us/dotnet/api/system.io.binaryreader.readdouble?view=netframework-4.7.2)
     */
    readChar(encoding = Encoding_1.Encoding.Utf8) {
        if (!(0, Encoding_1.isValidEncoding)(encoding)) {
     ...
     */
    readChars(charactersToRead, encoding = Encoding_1.Encoding.Utf8) {
        if (typeof charactersToRead !== 'number'
        ...
        else if (!(0, Encoding_1.isValidEncoding)(encoding)) {
            throw new EncodingError_1.EncodingError(ErrorMessageFactory_1.EncodingMessageFactory.unknownEncoding(encoding));
        }
     ...
     * @throws {@link InvalidUtf8CharacterError} Thrown when using UTF-8 encoding when an incorrect UTF-8 character sequence is encountered.
     */
    readCharBytes(bytesToRead, encoding = Encoding_1.Encoding.Utf8) {
        if (typeof bytesToRead !== 'number'
            || Number.isNaN(bytesToRead)
            || !Number.isFinite(bytesToRead)) {
            throw new InvalidArgumentError_1.InvalidArgumentError('`bytesToRead` is not a number', 'bytesToRead', bytesToRead);
        }
        else if (!(0, Encoding_1.isValidEncoding)(encoding)) {
            throw new EncodingError_1.EncodingError(ErrorMessageFactory_1.EncodingMessageFactory.unknownEncoding(encoding));
        }
     ...
     */
    readString(encoding = Encoding_1.Encoding.Utf8) {
        if (!(0, Encoding_1.isValidEncoding)(encoding)) {
            throw new EncodingError_1.EncodingError(ErrorMessageFactory_1.EncodingMessageFactory.unknownEncoding(encoding));
        }
        if (this.remainingBytes === 0) {
